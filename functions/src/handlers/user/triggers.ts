  ...
  async (event) => {
    try {
      ...
      if (before.displayName !== after.displayName) {
        ...
        updates.push({
          id: `${userId}_displayName_${now.getTime()}`,
          userId,
      ...
      if (before.iconUrl !== after.iconUrl) {
        ...
        updates.push({
          id: `${userId}_iconUrl_${now.getTime()}`,
          userId,
          fieldName: "iconUrl",
      ...
      while (retryCount < maxRetries) {
        try {
          console.log(`即時同期開始: ${userId} (試行 ${retryCount + 1}/${maxRetries})`);
          await processUserUpdates(userId, updates);
          console.log(`即時同期完了: ${userId}`);
          return;
        } catch (error) {
          retryCount++;
          lastError = error instanceof Error ? error : new Error(String(error));
          console.error(`即時同期エラー (試行 ${retryCount}/${maxRetries}):`, error);
      ...
      // 全てのリトライが失敗した場合、アラートを記録
      if (lastError) {
        console.error(`即時同期が全て失敗: ${userId}`, lastError);
